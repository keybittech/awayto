...
let seedPath = process.argv[2] || '';

const seedFile = path.isAbsolute(seedPath) ? JSON.parse(fs.readFileSync(seedPath)) : {
...
if (!seedPath) {
  seedFile.seed = (new Date).getTime();
  seedFile.awaytoId = `${seedFile.name}${seedFile.environment}${seedFile.seed}`
  seedPath = path.join(__dirname, `data/seeds/${seedFile.awaytoId}.json`);
}
...
try {
  ...
  const createRdsInstance = async () => {
    ...
    const createCommand = new CreateDBInstanceCommand({
      ...
      DBName: awaytoDb, // TODO custom name management; of multi db instances
