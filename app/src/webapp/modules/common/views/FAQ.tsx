...
export function FAQ(props: IProps): JSX.Element {
  const { classes } = props;
      ...
      <CardContent>
        <Grid container>
          <Grid item xs={12}>
            ...
            <Typography variant="subtitle1">If you feel that this list doesn't answer your questions, please <Typography className={classes.link} color="secondary" component={Link} to={{ pathname: "https://discord.gg/KzpcTrn5DQ" }} target="_blank">join the discord</Typography> to suggest changes.</Typography>
          </Grid>
          <Grid item xs={12}>
            <Typography variant="h6">What gets installed?</Typography>
            <Typography variant="subtitle1">You can review the <Typography className={classes.link} color="secondary" component={Link} to={{ pathname: "https://github.com/keybittech/awayto/blob/main/INSTALL.md" }} target="_blank">old installation guide</Typography> to get an idea of the more detailed changes the installation process applies to AWS. In general, after running the installer, the following resources will be deployed to AWS:</Typography>
            <List dense={true}>
              <ListItem><ListItemText primary="Cognito User Pool" disableTypography={true} /></ListItem>
              <ListItem><ListItemText primary="Cognito App Client" disableTypography={true} /></ListItem>
              <ListItem><ListItemText primary="RDS Postgres Instance (t3.micro)" disableTypography={true} /></ListItem>
              <ListItem><ListItemText primary="API Gateway stage containing authorized and unauthorized routes" disableTypography={true} /></ListItem>
              <ListItem><ListItemText primary="Lambda Function to receive gateway events" disableTypography={true} /></ListItem>
              <ListItem><ListItemText primary="Service worker 'LambdaTrust' role for lambda-intranet access" disableTypography={true} /></ListItem>
              <ListItem><ListItemText primary="S3 buckets for hosting built web and API bundles" disableTypography={true} /></ListItem>
              <ListItem><ListItemText primary="CloudFormation distribution to serve the webapp from S3" disableTypography={true} /></ListItem>
              <ListItem><ListItemText primary="Local file system containing a generic Lambda node-based API, Typescript type suite, and React web app originally generated with create-react-app." disableTypography={true} /></ListItem>
            </List>

            <Typography variant="h6">Where are things located in the file system?</Typography>
            <Typography variant="subtitle1">The project is divided into three main code bases:</Typography>

            <Typography variant="subtitle1"><strong>API:</strong> <pre>src/api</pre></Typography>
            <Typography variant="body1">The API is a generic Node JS structure which implements a Lambda function. The <pre>index.ts</pre> file collects all the db objects, and generates a set of routes defined by the type system. When called, the events will process inputs and return outputs as described by their associated type. In this way, the standard convention is that all API routes should be directly tied to some type within the system.</Typography>

            <Typography variant="subtitle1"><strong>Core:</strong> <pre>src/core</pre></Typography>
...
`}</pre></Typography>
            <Typography variant="subtitle1">With these structures in place, you would be able to now create API routes that link to those defined, as well as access the object through Redux.</Typography>
