...
import { CloudFrontClient, DeleteDistributionCommand, GetDistributionCommand, DeleteCloudFrontOriginAccessIdentityCommand, UpdateDistributionCommand, waitUntilDistributionDeployed, GetCloudFrontOriginAccessIdentityCommand, Distribution } from '@aws-sdk/client-cloudfront';

import { ask } from './tool';
import { asyncForEach } from './tool';
  ...
  if (!id) {
    ...
    if (seed.distributionId) {
      try {
        ...
        if (getDist?.Distribution?.DistributionConfig) {
          getDist.Distribution.DistributionConfig.Enabled = true;
